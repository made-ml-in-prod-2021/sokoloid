...
def test_predict(url: str):
    response = requests.get(url + "/predict/[55.80330934948255,%2049.33272207144011]", headers={"X-Token": "coneofsilence"})
    assert_1 = response.status_code == 200
    answer = response.content.decode(response.encoding)
    ...
    response = requests.get(url + "/predict/[55.79883508185922,%2049.105875912272566]", headers={"X-Token": "coneofsilence"})
